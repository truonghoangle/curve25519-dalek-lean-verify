...
/-!
# Spec theorem for `curve25519_dalek::scalar::Scalar::from` (From<u16>)

Constructs a `Scalar` from a `u16` value `x` by:
- Creating a 32-byte array initialized to zero
- Converting `x` to its 2-byte little-endian representation `x_bytes`
- Copying `x_bytes` into the first 2 bytes of the 32-byte array
- Wrapping the resulting 32-byte array as a `Scalar`

Because every `u16` value is less than 2¹⁶, and 2¹⁶ < L (the group order, ≈ 2²⁵²),
...
private lemma U16_ofDigits_toLEBytes (x : U16) :
  ...
  simp only [Nat.reducePow, Nat.reduceMod, BitVec.fromLEBytes_toLEBytes,
    BitVec.toNat_cast, U16.bv_toNat] at hdigits
  exact hmap.symm ▸ hdigits

private lemma U8x32_as_Nat_setSlice_zeroI (bs : List U8) (h_len : bs.length = 2) :
...
private lemma hmap (bs : List U8) (h_len : bs.length = 2) :
...
• The resulting `Scalar`'s byte representation, interpreted as a little-endian natural
  number via `U8x32_as_Nat`, equals `x.val` (the natural number value of `x`)
-/
@[step]
theorem from_spec (x : U16) :
  ...
  simp_all only [UScalarTy.U8_numBits_eq, Usize.ofNatCore_val_eq, Array.val_to_slice,
    ...
    Array.repeat_val, UScalar.ofNatCore_val_eq, List.reduceReplicate, List.slice_zero_j,
    List.take_succ_cons, List.take_zero, Slice.length, tsub_zero, List.length_cons,
    List.length_nil, zero_add, Nat.reduceAdd, Slice.setSlice!_val, List.length_setSlice!,
    ↓reduceDIte]
